...
Odd-coset realization splits into signed branch provenance, odd local packet square
(`0001`/missing-`0111`), or global unbranched sheet-character provenance.  In the branch case, two
opposite-parity realizations of one lower profile reduce to a minimal signed disjoint relation `A-A'`;
closure is exactly the promotion of that signed relation to an unsigned lower-zero packet.
Equivalently, in the exchange graph of realizations of one lower profile, opposite parities in one
component give an odd local cycle, while opposite parities in different components are just another
sheet-character provenance obstruction.
In the unbranched sheet case, any large quotient-uniform sheet component either gives a smaller selector or
restarts the pure `Q` target-avoid analysis inside the sheet; hence a terminal survivor is forced to be a
rank-one boundary-provenance failure for the sheet character.
That failure is equivalent to restart admissibility: a zero-residue prefix-local sheet separator must be
first-boundary admissible unless the first comparison with the original descent exposes a local
missing-corner square.  In the exchange-saturated boundary category this is tautological; the unsaturated
content is path-saturation equivalence.
The path-saturation gap has a shortest-loop normal form: a chordless rank-one flat loop with no nonfilled
or curved square, no filled-flat chord, no exchange-complete proper interval, and no repeated lower
profile.  The omni-saturation atom is the first such loop not generated by filled squares and complete
intervals.
Active-edge normalization then leaves only a residue-zero two-state sheet transposition not generated by
an admitted boundary exchange, the shared host endpoint behind missing two-fiber overlap, no-split failure,
and anchored one-corner lift failure.
After cancelling common parts, the transposition carrier is `K=A union A'` with
`delta_r(K)=deg_A(r)-deg_A'(r)`.  If every ambient row has `delta_r(K)=0`, this is a sheet-clone module; a
survivor has a first distinguishing row, and minimality makes it a fully-skew separator with no proper
distinguished subcarrier.  The endpoint is fully-skew row promotion unless the first row-action change
exposes the local `0001` square.
Once this row is admitted in `FR^sat`, Proposition 9.2 excludes it by the protected-packet side-size
argument (`0<side<q` but `side=0 mod q`).  Hence the live unsaturated task is only transporting the
path-provenance first fully-skew row to an `FR^sat` boundary row, or finding the local missing-corner square
during transport.
Equivalently, insert the fully-skew row at the front of the path-boundary word and commute it leftward:
every nonfilled/curved commutation is local, every flat commutation preserves the same carrier, and a
minimal survivor is a same-carrier skew ladder with identical visible data but hidden path memory.
...
For fixed `T,X`, the four `kappa` targets are coherent: with `q=deg_X-psi_T`, active coordinates are
`h_k=k+q in {1,2}`, zero coordinates are `h_k in {0,3}`, and switching uses
`sigma_k=1_{h_k in {2,3}}`; opposite residues swap active/zero and complement all traces.  Hence the four
filtered covers are generated by the same outside columns, not independent set systems.
If opposite residues are both high-active target-avoidance endpoints, singleton-shadow collapse forces an
antipodal trace core: two copies of `sigma`, two of `1-sigma`, and all singleton flips around both centers
on their active sides.  Otherwise one residue in the opposite pair is scalar-killed.
...
allowed interval.
Since the original coordinate certificate is inclusion-minimal, this extends to every proper shadow
`(A',Z')`: the full `(A,Z)` fails, but every proper active/filter shadow satisfies the filtered-cover table.
...
supports alone survive only in dimensions `5,6,7`: exactly a triangle for `|A|=5`, matching-number-2 with
all vertex deletions still matching-number-2 for `|A|=6`, and factor-critical for `|A|=7`.  From
`|A|>=8` a projected support of size at least three is forced.
With `mu_Z` the maximum full-family excess, every zero relaxation must add private supports whose net saving
