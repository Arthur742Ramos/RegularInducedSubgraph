...
The cover has three exits: signed branch provenance from opposite parities over one lower profile, an odd
local packet square, or an unbranched sheet-character provenance obstruction.
The signed branch folds into the other two once the fixed-profile exchange graph is used: connected
opposite parities give an odd local cycle, disconnected opposite parities give sheet provenance.
Large quotient-uniform sheet components reduce to smaller selector or smaller pure-`Q` analysis, so the
irreducible sheet case is boundary-provenance fullness for the rank-one character.
Equivalently, zero-residue prefix-local sheet separators must pass restart admissibility; only
path-saturation equivalence separates this from the exchange-saturated closure.
The remaining comparison is a shortest chordless rank-one flat loop with no local square exit and no
proper complete interval; omni-saturation is exactly the assertion that no such loop survives except as a
two-state square/local module exit.
After active-edge normalization, the only survivor is a residue-zero two-state sheet transposition not
generated by the admitted boundary exchanges.
Its row-action form is a carrier `K=A union A'` and first distinguishing row `r` with
`delta_r(K)=deg_A(r)-deg_A'(r) !=0` while all earlier rows have zero action; minimality makes `r` a
fully-skew separator.
The saturated convention already kills such rows by Proposition 9.2, so only path-to-`FR^sat` provenance
transport remains.
This transport has a prefix-insertion ladder form: flat commutations preserve a same-carrier fully-skew
...
Those are exactly the saturated local alternatives already closed, so the irreducible target-avoidance
...
Thus cross-atom supports have strict atom defect `delta>=pi`; in the four-four no-leftover subcase,
supports crossing two, three, or four atoms have size at most `6`, `9`, or `12`.
Deletion in this four-four no-leftover subcase is exactly unit strict absorption or lift-collision.
...
For size-two atoms, the noncollision branch is only a zero-gain pivot.
Collision stars remain genuine here: minimality gives positive marginal contribution, not positive individual
gain.
...
quotient-regular type-crossing edges.
Such an `M` edge is incompatible with all-ternary high-outdegree, because it would force the two remaining
atoms to be opposite in type to both endpoints.  Thus `F`-containing all-ternary endpoints are typed bipartite
`F` graphs with omitted-coordinate labels on corner edges: vertices of `F`-degree `0` need all three
