...
\[
  W_{2^r}(n) \ge \frac{n}{2 \prod_{j=1}^{r-1} (2^j)^C}
  = \frac{n}{2^{1 + C r(r-1)/2}}.
\]
Choose $r$ maximal with
\[
  1 + r + \frac{C r(r-1)}{2} \le \log_2 n.
\]
Then $2^r \le W_{2^r}(n)$. By (3), every graph on $n$ vertices therefore contains a regular
induced subgraph on $2^r$ vertices. Since $r = \Omega(\sqrt{\log n})$, we obtain
\[
...
any composable fixed-modulus package would also force the conjecture. That is the structural
