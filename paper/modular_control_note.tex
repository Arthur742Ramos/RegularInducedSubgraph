...
\author[D.~B.~Hulak]{David Barros Hulak}
\address{Independent researcher}
\email{dbhulak@gmail.com}

\author[R.~de Queiroz]{Ruy B.~J.~G. de Queiroz}
...
regular induced subgraph of the same order. A low-degree modular witness is also exact, since all
induced degrees lie in $\{0,1,\dots,q-1\}$ and are pairwise congruent modulo $q$.
\end{proof}
...
The reformulation above leaves one canonical equivalent problem, one stronger exact route, and one
conditional dyadic-lifting criterion.
...
Problem~\ref{prob:small-control} would imply that $F(n)\to\infty$: for each fixed $m$, every
sufficiently large graph would then contain a regular induced subgraph on at least $m$ vertices.
That qualitative conclusion alone does not recover the conjectured superlogarithmic lower bound,
because the threshold beyond which the witness appears may depend arbitrarily on $m$. Thus this
stronger problem is not claimed equivalent to the conjecture; it is included because it isolates one
natural exact-control route beyond the canonical modular frontier. The modular route suggests an
intermediate target: replace exact equalities by congruences modulo a modulus $q$ comparable to
$|S|$, or even more sharply by congruences modulo a modulus larger than the induced degree range,
and then appeal to Proposition~\ref{prop:modular-collapse}. In either form, the obstacle is no
longer definitional. The canonical open step is Problem~\ref{prob:canonical-frontier};
Problem~\ref{prob:small-control} asks whether one can do better by forcing a more rigid witness.

\subsection*{A dyadic modulus-lifting route}

The canonical frontier also suggests a quantitative modulus-by-modulus program. Let
$\mathcal{W}_{2^j}$ be any family of witness notions indexed by powers of two, where a witness in
$\mathcal{W}_{2^j}$ comes with a terminal vertex set on which all induced degrees are congruent
modulo $2^j$.

\begin{proposition}[dyadic lifting criterion]\label{prop:dyadic-criterion}
Assume that the family $(\mathcal{W}_{2^j})_{j \ge 1}$ has the following properties.
\begin{enumerate}
  \item Every graph on $n$ vertices has a witness in $\mathcal{W}_2$ of order at least $n/2$.
  \item There is an absolute constant $C$ such that for every $j \ge 1$ and every $m \in \N$,
    every graph with a witness in $\mathcal{W}_{2^j}$ of order at least $(2^j)^C m$ also has a
    witness in $\mathcal{W}_{2^{j+1}}$ of order at least $m$.
  \item Every witness in $\mathcal{W}_{2^j}$ of order at least $2^j$ yields a regular induced
    subgraph on $2^j$ vertices.
\end{enumerate}
Then $F(n) \ge \exp(\Omega(\sqrt{\log n}))$; in particular $F(n)/\log n \to \infty$.
\end{proposition}

\begin{proof}
For a power of two $q$, let $W_q(n)$ denote the largest integer $m$ such that every graph on $n$
vertices has a witness in $\mathcal{W}_q$ of order at least $m$. By (1), $W_2(n) \ge n/2$.
Iterating (2) from $2$ to $2^r$ gives
\[
  W_{2^r}(n) \ge \left\lfloor \frac{n}{2 \prod_{j=1}^{r-1} (2^j)^C} \right\rfloor
  = \left\lfloor \frac{n}{2^{1 + C r(r-1)/2}} \right\rfloor.
\]
Choose $r$ maximal with
\[
  1 + r + \frac{C r(r-1)}{2} \le \log_2 n.
\]
Then
\[
  \frac{n}{2^{1 + C r(r-1)/2}} \ge 2^r,
\]
so in particular $2^r \le W_{2^r}(n)$. By (3), every graph on $n$ vertices therefore contains a
regular induced subgraph on $2^r$ vertices. Since $r = \Omega(\sqrt{\log n})$, we obtain
\[
  F(n) \ge 2^r = \exp(\Omega(\sqrt{\log n})),
\]
and in particular $F(n)/\log n \to \infty$.
\end{proof}

Gallai's classical parity theorem gives the base case at modulus $2$ for the empty-control package
of induced subgraphs with all degrees even \cite{Gallai}. The remaining difficulty is therefore not
the first modulus, but finding a composable fixed-modulus witness notion for which the lift in
Proposition~\ref{prop:dyadic-criterion}(2) can actually be proved and for which the collapse in
(3) is available at the terminal scale.
...
exponentially many profile classes by polynomially many residue classes. Proposition~
\ref{prop:dyadic-criterion} records a separate conditional route: a polynomial-cost dyadic lift in
any composable fixed-modulus package would force the quantitative bound
$F(n) \ge \exp(\Omega(\sqrt{\log n}))$, which comfortably dominates the conjectured
superlogarithmic growth. That is the structural
...
\bibitem{Gallai}
T.~Gallai,
\newblock {\"U}ber extreme Punkt- und Kantenmengen,
\newblock {\em Annales Universitatis Scientiarum Budapestinensis de Rolando E{\"o}tv{\"o}s Nominatae.
Sectio Mathematica} \textbf{2} (1959), 133--138.
...
\url{https://github.com/Arthur742Ramos/RegularInducedSubgraph}, which is intended as
verification support rather than as part of the mathematical contribution. The authors
thank the Lean community for the \texttt{mathlib} infrastructure underlying that
formalization.\par}
