...
There is an immediate small-core cutoff.  Let `N_i={v in S:q(v)=i}`.  The active set for a residue `k` is
one adjacent pair of these four classes.  If all four residues are target-avoidance rather than scalar-killed,
the saturated local closures force
...
has at least one scalar-killed residue for each fixed `T,X`.
...
large-outside ternary target avoidance:
  ...
  opposite high-active endpoints; pure target-avoidance for all four residues forces m>=10;
