...
For fixed `T,X`, the four choices of final residue are coherent.  Put

```text
q(v)=deg_X(v)-psi_T(v),        h_k(v)=k+q(v)        [MOD 4].
```

The switching just described is performed at coordinate `v` exactly when `h_k(v) in {2,3}`.  Therefore

```text
A_k={v:h_k(v) in {1,2}},        Z_k={v:h_k(v) in {0,3}},
sigma_k(v)=1_{h_k(v) in {2,3}}.
```

An outside column `a_y(v)=1_{vy}` is admissible for the `k`-cover precisely when

```text
a_y(v)=sigma_k(v)        for every v in Z_k,
```

and its active support is `{v in A_k:a_y(v) != sigma_k(v)}`.  Opposite residues are exact complements:

```text
A_{k+2}=Z_k,        Z_{k+2}=A_k,        sigma_{k+2}=1-sigma_k.
```

Equivalently, writing `N_i={v:q(v)=i}`, the four covers are:

```text
k=0:  A=N_1 union N_2,  Z=N_0 union N_3,  sigma=1 on N_2 union N_3;
k=1:  A=N_0 union N_1,  Z=N_2 union N_3,  sigma=1 on N_1 union N_2;
k=2:  A=N_3 union N_0,  Z=N_1 union N_2,  sigma=1 on N_0 union N_1;
k=3:  A=N_2 union N_3,  Z=N_0 union N_1,  sigma=1 on N_3 union N_0.
```

Thus a terminal target-avoidance endpoint is not an arbitrary single filtered cover.  For the same `T,X`,
all four filtered covers are generated by the same outside trace columns under this cyclic active/filter
rotation; any high-active excess-packing obstruction must satisfy this four-residue coherence.

There is an immediate small-core cutoff for the high-active branch.  Let `N_i={v in S:q(v)=i}`.  The active
set for a residue `k` is one adjacent pair of these four classes.  If all four residues are in the
high-active target-avoidance branch, then

```text
|N_i|+|N_{i+1}| >= 4        for every i in Z/4Z.
```

Summing gives `|S|>=8`, i.e. `m>=10` for `|T|=m+1` and `|X|=3`.  Thus for `m<=9`, every ternary terminal
...
```text
at least two copies of sigma_k,        at least two copies of 1-sigma_k,
sigma_k xor e_a        for every a in A_k,
(1-sigma_k) xor e_z    for every z in Z_k,
```

where `e_p` flips the single coordinate `p`.  Thus opposite-residue target avoidance on the same certificate
forces two Hamming stars centered at complementary trace vectors, together with two repeated centers.  If
the two residues use different minimal certificates, the same statement holds after restricting to each
certificate separately; on their intersection the two descriptions are complementary restrictions of the
same outside trace multiset.  If this antipodal core is absent on a proposed shared certificate, then at
least one of the two residues is not a target-avoidance endpoint there and must be scalar-killed instead.
...
endpoint in the first few dimensions.  In pair-only language the excess is just matching number:

```text
n=5:  matching number 1, and every four-set has an edge; hence exactly a triangle;
n=6:  matching number 2, and every vertex deletion still has matching number 2;
n=7:  every vertex deletion has a perfect matching, i.e. a factor-critical seven-vertex graph.
```

For `n>=8`, pair supports cannot even make every active deletion feasible, so some projected support of
size at least `3` is forced.
...
Equivalently, for `|A|<=3` this is a finite zero-filter blocker.  List the allowed support triples from the
table above, including empty supports when needed.  For a candidate triple of outside vertices with those
active supports, let `Z_i` be the set of zero coordinates hit by the `i`-th vertex.  The full filter accepts
the triple exactly when

```text
Z_1 union Z_2 union Z_3 = emptyset.
```

Thus small-active target failure says every allowed support triple is hit by the blocker `Z_1 union Z_2
union Z_3`.  Full minor-criticality says every blocker coordinate is essential: for each `z in Z` there is
an allowed support triple with

```text
Z_1 union Z_2 union Z_3 = {z}.
```

So the small-active endpoint is not another infinite cover problem; it is a finite list of support
templates, each equipped with an essential zero-coordinate hitting certificate.
