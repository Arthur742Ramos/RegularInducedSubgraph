...
If both opposite residues `k` and `k+2` are in the high-active full-minor target-avoidance branch, the
singleton-shadow collapse gives an antipodal trace core.  On the common coordinate set, the outside trace
multiset contains
...
where `e_p` flips the single coordinate `p`.  Thus opposite-residue target avoidance forces two Hamming
stars centered at complementary trace vectors, together with two repeated centers.  If this antipodal core
is absent for some opposite pair, then at least one of the two residues is not a target-avoidance endpoint
and must be scalar-killed instead.
...
large-outside ternary target avoidance:
  ...
  opposite high-active endpoints;
